...
// File Name    :   i2cspibridge_spi_rd_test_seq.svh
// Description  :   I2C-SPI Bridge SPI Read Test Sequence
//-------------------------------------------------------------
`ifndef _I2CSPIBRIDGE_SPI_RD_TEST_SEQ_SVH
  `define _I2CSPIBRIDGE_SPI_RD_TEST_SEQ_SVH

class i2cspibridge_spi_rd_test_seq extends i2cspibridge_base_seq;

  // Register with the factory
  `uvm_object_param_utils_begin(i2cspibridge_spi_rd_test_seq)
  ...
  *************************************************************/
  function new(string name="i2cspibridge_spi_rd_test_seq");
    super.new(name);
    ...
    fork
      ...
      begin : stimulus_th
        ...
        forever begin
          ...
          `uvm_info("SPI RD TEST SEQ", "Randomizing Parameters", UVM_MEDIUM)
          randomize_test_params();

          // Step 2&3: Configure the RegMap and Send Stop Condition
          `uvm_info("SPI RD TEST SEQ", "Configuring RegMap", UVM_MEDIUM)
          ...
          `uvm_info("SPI RD TEST SEQ", "Reading from SPI SLV", UVM_MEDIUM)
          read_data_seq.addr       = {spi_slave, rd_addr};
          read_data_seq.read_iter = iter;
          read_data_seq.start(p_sequencer);
          `uvm_info("SPI RD TEST SEQ", "Finished Read", UVM_MEDIUM)
          
          // Step 8: Reset the I2C-SPI Bridge
          if(reset_bridge) begin
            `uvm_info("SPI RD TEST SEQ", "Resetting I2C-SPI Bridge", UVM_MEDIUM)
          end
      ...
      begin : timeout_th
        #(TEST_TIMEOUT );
        `uvm_info("SPI RD TEST SEQ", "Test Ended!", UVM_MEDIUM)
      end   : timeout_th
    join_any
  endtask : body
...
endclass : i2cspibridge_spi_rd_test_seq

`endif // _I2CSPIBRIDGE_SPI_RD_TEST_SEQ_SVH